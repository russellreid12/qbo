...
time.sleep(0.02)
# controller.SetPid(1, 32, 0, 0)
time.sleep(0.02)
# controller.SetPid(2, 32, 0, 0)
time.sleep(0.02)
...
if not os.path.exists("./haarcascades/face_frames/"):
	os.makedirs("./haarcascades/face_frames/")

while dir_exist:
	if not os.path.exists("./haarcascades/face_frames/" + str(dir_idx)):
		os.makedirs("./haarcascades/face_frames/" + str(dir_idx))
		dir_exist = False
	else:
		dir_idx += 1
...
while True:
	...
	if not faceFound:  # if no face was found...-
		...
		cv2.imwrite("./haarcascades/face_frames/" + str(dir_idx) + "/frm" + str(fr_time_ms) + ".jpg", aframe)  # save frame as JPEG $
	...
	if time.time() - touch_samp > 0.5:
		...
		if touch_tm == 0 and qbo_touch:
			if qbo_touch == [1]:
				controller.SetServo(1, Xmax - 25, 100)
				time.sleep(0.1)
				controller.SetServo(2, Ymin - 5, 100)
				WaitTouchMove()
				...
				controller.SetServo(1, Xmin + 25, 100)
				time.sleep(0.1)
				controller.SetServo(2, Ymin - 5, 100)
				WaitTouchMove()
