...
class Controller(object):
  ...
  def ReadResponse(self):
      """Assemble one framed packet (0xFF ... 0xFE). Pi 5 / busy boards need >250ms total."""
      orig_timeout = self.port.timeout
      self.port.timeout = 0.05
      isInputEscaped = False
      bExit = False
      wait_start = time.time()
      deadline = wait_start + 2.0
      rx_data = []
      try:
          while time.time() < deadline:
              nwait = self.port.in_waiting
              to_read = min(128, nwait) if nwait else 1
              rdBuffer = self.port.read(to_read)
              for x in rdBuffer:
  ...
  def GetHeadCmd(self, cmd, cmd_buffer):
      ...
      if len(rxBuffer) < 5:
          now = time.time()
          if now - self._serial_warn_ts >= 20.0:
              self._serial_warn_ts = now
              print(
                  "Serial: no valid response for %s (got %d bytes). Check serialPort / UART / PiCmd. "
                  "(Repeating messages suppressed for 20s.)" % (cmd, len(rxBuffer))
              )
